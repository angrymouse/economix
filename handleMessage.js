module.exports = async (message) => {
  ...
  util.updateProfile(message.author.id, message.guild.id, {
    money: {
      ...memberProfile.money,
      cash: memberProfile.money.cash + util.random(serverProfile.paymentPerMessage.from, serverProfile.paymentPerMessage.to)
    }
  });
  if (message.content.startsWith(serverProfile.prefix)) {
    ...
    if (fs.existsSync(`./commands/${command}.js`)) {
      ...
      if (!commandModule.requiredPermissions) {
        commandModule.requiredPermissions = []
      }
